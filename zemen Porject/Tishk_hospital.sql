...
new_appointment_type varchar(200),
old_appointment_type varchar(200),
log_Date date,
log_Time time
...
id int primary key auto_increment,
room_id int,
new_room_type varchar(200) ,
old_room_type varchar(200),
new_max_capacity int,
old_max_capacity int ,
new_availability tinyint(1) ,
old_availibility tinyint(1),
actionn varchar(20),
log_Date date,
log_Time time
...
-- trigger queries
 ...
 -- insert trigger for appoinments
 
 
create trigger appointments_insertaion
...
actionn = "insertaion",
new_appointment_date = new.appointment_date,
new_appointment_time = new.appointment_time,
new_appointment_type = new.appointment_type,
log_date = current_date(),
log_time = current_time();

-- delete trigger for appointments
...
actionn = "delete",
new_appointment_date = old.appointment_date,
new_appointment_time = old.appointment_time,
new_appointment_type = old.appointment_type,
log_date = current_date(),
log_time = current_time();
...
new_appointment_date = new.appointment_date,
old_appointment_time = old.appointment_time,
new_appointment_time = new.appointment_time,
old_appointment_type = old.appointment_type,
new_appointment_type = new.appointment_type,
log_date = current_date(),
log_time = current_time();


-- end of appoinments trigger queries

-- trigger for room log

 -- insert trigger for room
 
 
create trigger room_insertaion
after insert on room
for each row
insert into room_log
set
room_id = new.room_id,
new_room_type = new.room_type,
 new_max_capacity= new.max_capacity,
...
create trigger room_deleting
after delete on room
for each row
insert into room_log
set
room_id = old.room_id,
old_room_type = old.room_type,
old_max_capacity= old.max_capacity,
 old_availibility = old.availability,
 actionn = "delete",
log_date = current_date(),
log_time = current_time();
...
old_room_type = old.room_type,
 new_max_capacity= new.max_capacity,
  old_max_capacity= new.max_capacity,
 new_availability = new.availability,
  old_availibility = old.availability,
 actionn = "update",
log_date = current_date(),
...
insert into room 
values ( 209 , "meeting" , 10, 1);

update room
set room_id = 209 , room_type = "surgery", max_capacity = 2
where room_id = 209;

delete from room
where room_id = 209;
-- end of aroom trigger queries
...
-- to see only patients and their names , dates and appointment status .. for appointment 

create view patient_appointment as
select appointments.patient_id , fname , sname , type_of_disase,appointment_date,app_room,appointment_status
from appointments,patient
...
-- to see doctors detail with those patient which they have to do appointments with

create view doctor_patient_details as
select doc_id , fname , sname , week_availability , room , patiient_id
from patient_doctor,doctors
where doctors.doc_id = patient_doctor.doctor_id ;


select * from  doctor_patient_details;
